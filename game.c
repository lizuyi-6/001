...
// 初始化棋盘
void InitBoard(char board[][COLS], int r, int c, char set)
{
    int i = 0;
    for (i = 0; i < r; i++)
    {
...
// 递归展开无雷区域
static void Expand(char mine[][COLS], char show[][COLS], int x, int y, int r, int c)
{
    // 1. 检查坐标是否在有效范围（1~r行，1~c列）
    if (x < 1 || x > r || y < 1 || y > c)
        return;
...
// 扫雷核心逻辑（新增mine_count参数适配自定义模式）
void FindMine(char mine[][COLS], char show[][COLS], int r, int c, int mine_count)
{
    int x = 0;
    int y = 0;
    int win = 0;  // 已扫出的非地雷数量
